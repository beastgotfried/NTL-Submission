...
def train_model(csv_file, plot= True): #creating a function to use in the other file

    #importing the csv dataset as a pandas dataframe

    data=pd.read_csv('Salary_Data.csv')

    #now we need to find a relationship between YOE and salary to plot a graph
    #gathering infomartion of x and y

    x=data['YearsExperience'].values
    y=data['Salary'].values

    #now we have collected the values of the experience and salary into our variables x and y
    #here they will store the data in one long array
    ...
    #so now the equation of a line is y= mx+c where m is the slope and c is the intercept
    #to find m we will use the formula of the slope of the least squares of a regression line

    #finding number of input values
    i= len(x)
